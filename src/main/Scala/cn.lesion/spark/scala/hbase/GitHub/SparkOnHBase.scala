package cn.lesion.spark.scala.hbase.GitHub
...
object SparkOnHBase {
  ...
  def main(args: Array[String]) {
    ...
    val spconf = new SparkConf().setAppName("Spark WordCount").setMaster("local[*]")
    val spark = new SparkContext(spconf)
    val hbconf = HBaseConfiguration.create()
    hbconf.set("hbase.zookeeper.property.clientPort", "2181")
    hbconf.set("hbase.zookeeper.quorum", "master")
    // ======Save RDD to HBase========
    // step 1: JobConf setup
    val jobConf = new JobConf(hbconf, this.getClass)
    ...
    //设置查询的表名
    hbconf.set(TableInputFormat.INPUT_TABLE, "user")
    //添加过滤条件，年龄大于 18 岁
    val scan = new Scan()
    scan.setFilter(new SingleColumnValueFilter("basic".getBytes, "age".getBytes,
      CompareOp.GREATER_OR_EQUAL, Bytes.toBytes(18)))
    hbconf.set(TableInputFormat.SCAN, convertScanToString(scan))
    val usersRDD = sc.newAPIHadoopRDD(hbconf, classOf[TableInputFormat],
